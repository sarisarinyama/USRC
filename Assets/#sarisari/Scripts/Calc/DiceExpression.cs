    ...
    public class DiceExpression
    {
        ...
        public DiceExpression(string expression, DiceExpressionOptions options)
        {
            ...
            for (int tokenIndex = 0; tokenIndex < tokens.Length; tokenIndex += 2)
            {
                ...
                {
                    this.nodes.Add(
                        new KeyValuePair<int, IDiceExpressionNode>(multiplier, new NumberNode(int.Parse(nextToken))));
                }
                else if (DiceExpression.diceRollToken.IsMatch(nextToken))
                {
                    ...
                    this.nodes.Add(new KeyValuePair<int, IDiceExpressionNode>(multiplier,
            ...
            if (options == DiceExpressionOptions.SimplifyStringValue)
            {
                ...
                var normalizedDiceRollNodes = from type in diceTypes
                    let numDiceOfThisType = diceRollNodes.Where(node => ((DiceRollNode) node.Value).DiceType == type)
                        .Sum(node => node.Key * ((DiceRollNode) node.Value).NumberOfDice)
